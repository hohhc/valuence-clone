            ...
            <ul class="flex flex-row flex-wrap lg:flex-col gap-x-4 gap-y-2 border-b lg:border-b-0 border-gray-200 pb-5 lg:pb-0">
              <li v-for="cat in categories" :key="cat.value">
                <button
                  ...
                  :class="selected === cat.value ? 'text-black font-bold' : 'text-gray-medium hover:text-black'"
                  @click="selected = cat.value"
                >
                  {{ cat.label }}
                </button>
              </li>
...
const selectedLabel = computed(() => categories.find((c) => c.value === selected.value)?.label || 'All')
const filteredNews = computed(() =>
  selected.value === 'all' ? newsItems : newsItems.filter((n) => n.category === selected.value)
)

useHead({ title: 'News | Valuence Holdings' })
</script>
